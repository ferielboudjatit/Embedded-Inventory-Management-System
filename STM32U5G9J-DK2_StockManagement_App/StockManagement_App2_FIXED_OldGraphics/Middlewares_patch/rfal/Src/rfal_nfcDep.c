    ...
    if( gNfcip.rxBuf[msgIt] == (uint8_t)NFCIP_CMD_PSL_REQ )
    {
        ...
        *outBRS = gNfcip.rxBuf[msgIt++];                  /* assign output BRS value      */
        ...
        if( nfcDepDev != NULL )
        {
            ...
            nfcDepDev->info.rfalDSI = (rfalBitRate)rfalNfcDepBRS2DSI( *outBRS );   /* DSI codes the bit rate from Initiator to Target */
        ...
        rfalSetBitRate( RFAL_BR_KEEP, gNfcip.nfcDepDev->info.rfalDSI );
...
ReturnCode rfalNfcDepInitiatorHandleActivation( rfalNfcDepAtrParam* param, rfalBitRate desiredBR, rfalNfcDepDevice* nfcDepDev )
{
    ...
    rfalGetBitRate( &nfcDepDev->info.rfalDSI, &nfcDepDev->info.rfalDSI );
    ...
    /*******************************************************************************/
    sendPSL = false;
    PSL_BRS = rfalNfcDepDx2BRS( nfcDepDev->info.rfalDSI );  /* Set current bit rate divisor on both directions  */
    ...
    * */
    ...
    /* Check Baud rates                                                            */
    /*******************************************************************************/
    if( (nfcDepDev->info.rfalDSI != desiredBR) && (desiredBR != RFAL_BR_KEEP) )    /* if desired BR is different    */
    {
    ...
    if( sendPSL )
    {
        ...
        if( nfcDepDev->info.rfalDSI != desiredBR )
        {
            /* Check if device was in Passive NFC-A and went to higher bit rates, use NFC-F */
            if( (nfcDepDev->info.rfalDSI == RFAL_BR_106) && (gNfcip.cfg.commMode == RFAL_NFCDEP_COMM_PASSIVE) )
            {
            ...
            nfcDepDev->info.rfalDSI  = desiredBR;  /* DRI Bit Rate coding from Target to Initiator   */
            
            rfalSetBitRate( nfcDepDev->info.rfalDSI, nfcDepDev->info.DRI );
        }
...
ReturnCode rfalNfcDepListenStartActivation( const rfalNfcDepTargetParam *param, const uint8_t *atrReq, uint16_t atrReqLength, rfalNfcDepListenActvParam rxParam )
{
    ...
    rfalGetBitRate( &rxParam.nfcDepDev->info.rfalDSI, &rxParam.nfcDepDev->info.DRI );
    ...
    switch (err)
    {
        case RFAL_ERR_NONE:
            ...
            if( BRS != RFAL_NFCDEP_BRS_MAINTAIN )
            {
                ...
                {
                    RFAL_EXIT_ON_ERR( err, rfalSetMode( RFAL_MODE_LISTEN_ACTIVE_P2P, gNfcip.nfcDepDev->info.DRI, gNfcip.nfcDepDev->info.rfalDSI ));
                }
                else
                {
                    RFAL_EXIT_ON_ERR( err, rfalSetMode( ((RFAL_BR_106 == gNfcip.nfcDepDev->info.DRI) ? RFAL_MODE_LISTEN_NFCA : RFAL_MODE_LISTEN_NFCF), gNfcip.nfcDepDev->info.DRI, gNfcip.nfcDepDev->info.rfalDSI ));
                }
            }
            break;
