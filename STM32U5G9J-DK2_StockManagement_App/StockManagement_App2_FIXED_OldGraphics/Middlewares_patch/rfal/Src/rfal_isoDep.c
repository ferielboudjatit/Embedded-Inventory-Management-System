...
static void rfalIsoDepApdu2IBLockParam( rfalIsoDepApduTxRxParam apduParam, rfalIsoDepTxRxParam *iBlockParam, uint16_t txPos, uint16_t rxPos );
    ...
    static uint32_t rfalIsoDepSFGI2SFGT( uint8_t sfgi );
        ...
        static ReturnCode rfalIsoDepStartPPS( uint8_t DID, rfalBitRate rfalDSI, rfalBitRate DRI, rfalIsoDepPpsRes *ppsRes );
        static ReturnCode rfalIsoDepGetPPSSTatus( void );
    #endif /* RFAL_FEATURE_NFCA */

    #if RFAL_FEATURE_NFCB
        static ReturnCode rfalIsoDepStartATTRIB( const uint8_t* nfcid0, uint8_t PARAM1, rfalBitRate rfalDSI, rfalBitRate DRI, rfalIsoDepFSxI FSDI, uint8_t PARAM3, uint8_t DID, const uint8_t* HLInfo, uint8_t HLInfoLen, uint32_t fwt, rfalIsoDepAttribRes *attribRes, uint8_t *attribResLen );
...
ReturnCode rfalIsoDepListenStartActivation( rfalIsoDepAtsParam *atsParam, const rfalIsoDepAttribResParam *attribResParam, const uint8_t *buf, uint16_t bufLen, rfalIsoDepListenActvParam actParam)
{
    ...
    if( actParam.isoDepDev != NULL )
    {
        ...
        actParam.isoDepDev->info.rfalDSI  = gIsoDep.txBR;
    ...
    if( gIsoDep.state == ISODEP_ST_PICC_ACT_ATS )
    {
        ...
        if( ( ((uint8_t*)gIsoDep.actvParam.rxBuf)[RFAL_ISODEP_PPS_STARTBYTE_POS] & RFAL_ISODEP_PPS_MASK) == RFAL_ISODEP_PPS_SB )
        {
            ...
            if( dri != RFAL_BR_KEEP )
            {
                ...
                {
                    gIsoDep.actvParam.isoDepDev->info.rfalDSI =  dsi;
...
static ReturnCode rfalIsoDepStartPPS( uint8_t DID, rfalBitRate rfalDSI, rfalBitRate DRI, rfalIsoDepPpsRes *ppsRes )
{
    ...
    if( (ppsRes == NULL) || (rfalDSI > RFAL_BR_848) || (DRI > RFAL_BR_848) || (DID > RFAL_ISODEP_DID_MAX) )
    {
        return RFAL_ERR_PARAM;
    }
    ...
    gIsoDep.actv.ppsReq.PPS1 = (RFAL_ISODEP_PPS_PPS1 | ((((uint8_t)rfalDSI<<RFAL_ISODEP_PPS_PPS1_DSI_SHIFT) | (uint8_t)DRI) & RFAL_ISODEP_PPS_PPS1_DXI_MASK)); 
...
/*******************************************************************************/
ReturnCode rfalIsoDepPPS( uint8_t DID, rfalBitRate rfalDSI, rfalBitRate DRI, rfalIsoDepPpsRes *ppsRes )
{
    ReturnCode ret;
    
    RFAL_EXIT_ON_ERR( ret, rfalIsoDepStartPPS(DID, rfalDSI, DRI, ppsRes) );
...
static ReturnCode rfalIsoDepStartATTRIB( const uint8_t* nfcid0, uint8_t PARAM1, rfalBitRate rfalDSI, rfalBitRate DRI, rfalIsoDepFSxI FSDI, uint8_t PARAM3, uint8_t DID, const uint8_t* HLInfo, uint8_t HLInfoLen, uint32_t fwt, rfalIsoDepAttribRes *attribRes, uint8_t *attribResLen )
{
    ...
    if( (attribRes == NULL) || (attribResLen == NULL) || (rfalDSI > RFAL_BR_848) || (DRI > RFAL_BR_848) || (DID > RFAL_ISODEP_DID_MAX) )
    {
        return RFAL_ERR_NONE;
    }
    ...
    gIsoDep.actv.attribReq.Param.PARAM2 = ( ((((uint8_t)rfalDSI<<RFAL_ISODEP_ATTRIB_PARAM2_DSI_SHIFT) | ((uint8_t)DRI<<RFAL_ISODEP_ATTRIB_PARAM2_DRI_SHIFT)) & RFAL_ISODEP_ATTRIB_PARAM2_DXI_MASK) | ((uint8_t)FSDI & RFAL_ISODEP_ATTRIB_PARAM2_FSDI_MASK) );
...
/*******************************************************************************/
ReturnCode rfalIsoDepATTRIB( const uint8_t* nfcid0, uint8_t PARAM1, rfalBitRate rfalDSI, rfalBitRate DRI, rfalIsoDepFSxI FSDI, uint8_t PARAM3, uint8_t DID, const uint8_t* HLInfo, uint8_t HLInfoLen, uint32_t fwt, rfalIsoDepAttribRes *attribRes, uint8_t *attribResLen )
{
    ReturnCode ret;
    
    RFAL_EXIT_ON_ERR( ret, rfalIsoDepStartATTRIB( nfcid0, PARAM1, rfalDSI, DRI, FSDI, PARAM3, DID, HLInfo, HLInfoLen, fwt, attribRes, attribResLen ) );
...
ReturnCode rfalIsoDepPollAStartActivation( rfalIsoDepFSxI FSDI, uint8_t DID, rfalBitRate maxBR, rfalIsoDepDevice *rfalIsoDepDev )
{
    ...
    rfalIsoDepDev->info.rfalDSI = maxBR;
    ...
    switch( gIsoDep.state )
    {
        ...
        case ISODEP_ST_PCD_ACT_RATS:
            ...
            if( ret != RFAL_ERR_BUSY )
            {
                ...
                {
                    maxBR = gIsoDep.actvDev->info.rfalDSI;             /* Retrieve requested max bitrate */
                    ...
                    gIsoDep.actvDev->info.rfalDSI  = RFAL_BR_106;
                    ...
                    if( gIsoDep.actvDev->activation.A.Listener.ATS.TL > RFAL_ISODEP_ATS_MIN_LEN )
                    {
                        ...
                        {
                            rfalIsoDepCalcBitRate( maxBR, ((uint8_t*)&gIsoDep.actvDev->activation.A.Listener.ATS)[msgIt++], &gIsoDep.actvDev->info.rfalDSI, &gIsoDep.actvDev->info.DRI );
                        }
                    ...
                    if( (gIsoDep.actvDev->info.rfalDSI != RFAL_BR_106) || (gIsoDep.actvDev->info.DRI != RFAL_BR_106) )
                    {
                        /* Send PPS */ /*  PRQA S 0310 1 # MISRA 11.3 - Intentional safe cast to avoiding buffer duplication */
                        RFAL_EXIT_ON_ERR( ret, rfalIsoDepStartPPS( gIsoDep.actvDev->info.DID, gIsoDep.actvDev->info.rfalDSI, gIsoDep.actvDev->info.DRI, (rfalIsoDepPpsRes*)&gIsoDep.ctrlBuf ));
        ...
        case ISODEP_ST_PCD_ACT_PPS:
            ...
            if( ret != RFAL_ERR_BUSY )
            {
                ...
                if( ret == RFAL_ERR_NONE )
                {
                    ...
                    rfalSetBitRate( gIsoDep.actvDev->info.DRI, gIsoDep.actvDev->info.rfalDSI );
                }
                else
                {
...
ReturnCode rfalIsoDepPollBStartActivation( rfalIsoDepFSxI FSDI, uint8_t DID, rfalBitRate maxBR, uint8_t PARAM1, const rfalNfcbListenDevice *nfcbDev, const uint8_t* HLInfo, uint8_t HLInfoLen, rfalIsoDepDevice *rfalIsoDepDev )
{
    ...
    rfalIsoDepCalcBitRate( maxBR, nfcbDev->sensbRes.protInfo.BRC, &rfalIsoDepDev->info.rfalDSI, &rfalIsoDepDev->info.DRI );
...
ReturnCode rfalIsoDepPollHandleSParameters( rfalIsoDepDevice *rfalIsoDepDev, rfalBitRate maxTxBR, rfalBitRate maxRxBR )
{
    ...
    rfalIsoDepDev->info.rfalDSI = rxBR;
